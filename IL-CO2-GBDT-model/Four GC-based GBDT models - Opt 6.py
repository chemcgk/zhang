# python code of four GC-based GBDT models - Opt. 6
...
param_distributions = {
    ...
    'CatBoost': {
        ...
        'depth': [6, 7, 8],
        'l2_leaf_reg': [3, 5, 7],
        'random_strength': [1.0, 1.5, 2.0],
        'bagging_temperature': [0.8, 1.0, 1.2],
        'leaf_estimation_iterations': [3, 5, 7],
...
if results:
    ...
    for model_name, result in results.items():
        base_name = f"GC-{model_name} Opt. 6"
        print(f"\n💾 Saved {base_name} ...")
...
def generate_model_visualizations(model_name, model_info, y_train, y_test, save_path="."):
    ...
    plt.title(f"Pred. vs Exp. Capacity (GC-{model_name} Opt. 6)", fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename1 = f"Pred vs Exp Capacity (GC-{model_name} Opt. 6).png"
    ...
    plt.title(f'Residual Plot (GC-{model_name} Opt. 6)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename2 = f"Residual Plot (GC-{model_name} Opt. 6).png"
    ...
    plt.title(f'Error Distribution (GC-{model_name} Opt. 6)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename4 = f"Error Distribution (GC-{model_name} Opt. 6).png"
    ...
    print(f"\n   GC-{model_name} Opt. 6 Detailed Statistics:")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n📈 Generating visual charts for GC-{model_name} Opt. 6...")
...
def create_shap_summary_plot(model_name, shap_values, X_sample, feature_columns, save_path="."):
    try:
        ...
        plt.title(f'SHAP Summary Plot (GC-{model_name} Opt. 6)', 
        ...
        filename = f"SHAP Summary Plot (GC-{model_name} Opt. 6).png"
        full_path = os.path.join(save_path, filename)
...
def create_shap_importance_plot(model_name, shap_values, feature_columns, save_path="."):
    try:
        ...
        shap_importance_filename = f"SHAP Feature Importance (GC-{model_name} Opt. 6).xlsx"
        ...
        ax.set_title(f'SHAP Feature Importance (GC-{model_name} Opt. 6)', 
                    fontsize=title_fontsize, fontweight='bold', fontname='Arial')
        ...
        filename = f"SHAP Feature Importance (GC-{model_name} Opt. 6).png"
        full_path = os.path.join(save_path, filename)
        plt.savefig(full_path, dpi=300, bbox_inches='tight')
        plt.show()
        print(f"   📊 Saved: {filename}")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n🔬 Analyzing GC-{model_name} Opt. 6 model...")
        ...
        try:
            ...
            if summary_success and importance_success:
                print(f"   ✅ GC-{model_name} Opt. 6 SHAP analysis completed")
            else:
                print(f"   ⚠️ GC-{model_name} Opt. 6 SHAP analysis had issues")
                
        except Exception as e:
            print(f"   ❌ GC-{model_name} Opt. 6 SHAP analysis failed: {e}")
            import traceback
            print(f"   Detailed error: {traceback.format_exc()}")
