# python code of four GC-based GBDT models - Opt. 2
...
param_distributions = {
    ...
    'CatBoost': {
        ...
        'leaf_estimation_iterations': [2, 3, 5],
...
if results:
    ...
    for model_name, result in results.items():
        base_name = f"GC-{model_name} Opt. 2"
        print(f"\n💾 Saved {base_name} ...")
...
def generate_model_visualizations(model_name, model_info, y_train, y_test, save_path="."):
    ...
    plt.title(f"Pred. vs Exp. Capacity (GC-{model_name} Opt. 2)", fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename1 = f"Pred vs Exp Capacity (GC-{model_name} Opt. 2).png"
    ...
    plt.title(f'Residual Plot (GC-{model_name} Opt. 2)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename2 = f"Residual Plot (GC-{model_name} Opt. 2).png"
    ...
    plt.title(f'Error Distribution (GC-{model_name} Opt. 2)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename4 = f"Error Distribution (GC-{model_name} Opt. 2).png"
    ...
    print(f"\n   GC-{model_name} Opt. 2 Detailed Statistics:")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n📈 Generating visual charts for GC-{model_name} Opt. 2...")
...
def create_shap_summary_plot(model_name, shap_values, X_sample, feature_columns, save_path="."):
    try:
        ...
        plt.title(f'SHAP Summary Plot (GC-{model_name} Opt. 2)', 
        ...
        filename = f"SHAP Summary Plot (GC-{model_name} Opt. 2).png"
        full_path = os.path.join(save_path, filename)
...
def create_shap_importance_plot(model_name, shap_values, feature_columns, save_path="."):
    try:
        ...
        shap_importance_filename = f"SHAP Feature Importance (GC-{model_name} Opt. 2).xlsx"
        ...
        ax.set_title(f'SHAP Feature Importance (GC-{model_name} Opt. 2)', 
                    fontsize=title_fontsize, fontweight='bold', fontname='Arial')
        ...
        filename = f"SHAP Feature Importance (GC-{model_name} Opt. 2).png"
        full_path = os.path.join(save_path, filename)
        plt.savefig(full_path, dpi=300, bbox_inches='tight')
        plt.show()
        print(f"   📊 Saved: {filename}")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n🔬 Analyzing GC-{model_name} Opt. 2 model...")
        ...
        try:
            ...
            if summary_success and importance_success:
                print(f"   ✅ GC-{model_name} Opt. 2 SHAP analysis completed")
            else:
                print(f"   ⚠️ GC-{model_name} Opt. 2 SHAP analysis had issues")
                
        except Exception as e:
            print(f"   ❌ GC-{model_name} Opt. 2 SHAP analysis failed: {e}")
            import traceback
            print(f"   Detailed error: {traceback.format_exc()}")
