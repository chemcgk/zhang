# python code of four MD-based GBDT models - Opt. 3
...
if results:
    ...
    for model_name, result in results.items():
        base_name = f"MD-{model_name} Opt. 3"
        print(f"\n💾 Saved {base_name} ...")
...
def generate_model_visualizations(model_name, model_info, y_train, y_test, save_path="."):
    ...
    plt.title(f"Pred. vs Exp. Capacity (MD-{model_name} Opt. 3)", fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename1 = f"Pred vs Exp Capacity (MD-{model_name} Opt. 3).png"
    ...
    plt.title(f'Residual Plot (MD-{model_name} Opt. 3)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename2 = f"Residual Plot (MD-{model_name} Opt. 3).png"
    ...
    plt.title(f'Error Distribution (MD-{model_name} Opt. 3)', fontsize=title_fontsize, fontweight='bold', fontname='Arial')
    ...
    filename4 = f"Error Distribution (MD-{model_name} Opt. 3).png"
    ...
    print(f"\n   MD-{model_name} Opt. 3 Detailed Statistics:")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n📈 Generating visual charts for MD-{model_name} Opt. 3...")
...
def create_shap_summary_plot(model_name, shap_values, X_sample, feature_columns, save_path="."):
    try:
        ...
        plt.title(f'SHAP Summary Plot (MD-{model_name} Opt. 3)', 
        ...
        filename = f"SHAP Summary Plot (MD-{model_name} Opt. 3).png"
        full_path = os.path.join(save_path, filename)
...
def create_shap_importance_plot(model_name, shap_values, feature_columns, save_path="."):
    try:
        ...
        shap_importance_filename = f"SHAP Feature Importance (MD-{model_name} Opt. 3).xlsx"
        ...
        ax.set_title(f'SHAP Feature Importance (MD-{model_name} Opt. 3)', 
                    fontsize=title_fontsize, fontweight='bold', fontname='Arial')
        ...
        filename = f"SHAP Feature Importance (MD-{model_name} Opt. 3).png"
        full_path = os.path.join(save_path, filename)
        plt.savefig(full_path, dpi=300, bbox_inches='tight')
        plt.show()
        print(f"   📊 Saved: {filename}")
...
if results:
    ...
    for model_name, model_info in results.items():
        print(f"\n🔬 Analyzing MD-{model_name} Opt. 3 model...")
        ...
        try:
            ...
            if summary_success and importance_success:
                print(f"   ✅ MD-{model_name} Opt. 3 SHAP analysis completed")
            else:
                print(f"   ⚠️ MD-{model_name} Opt. 3 SHAP analysis had issues")
                
        except Exception as e:
            print(f"   ❌ MD-{model_name} Opt. 3 SHAP analysis failed: {e}")
            import traceback
            print(f"   Detailed error: {traceback.format_exc()}")
